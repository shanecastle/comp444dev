...
void setupSound() {
  logDebug("[SOUND] Setting up USB device");
  // Enable power for HOST USB connector
  pinMode(PA_15, OUTPUT);
  digitalWrite(PA_15, HIGH);

  logDebug("Connect a USB stick to the GIGA's USB port ...");
  while (!msd.connect()) delay(100);

  logDebug("Mounting USB device ...");
  ...
  logDebug("Opening audio file ...");
  // 16-bit PCM Mono 16kHz realigned noise reduction
  file = fopen("/USB_DRIVE/champion.wav", "rb");
  if (file == nullptr) {
...
void configFile(String filename) {
  Serial.println("Opening audio file ..." + filename);
