...
int previousState = -1;  // hold switch position

//VARIABLES
int motorSpeed = 100;  //starting speed for the motor
...
void loop() {
  ...
  if (digitalRead(7) == LOW) {  //if the switch is on...
    if (previousState != 1) {
      Serial.println("Switch is on. Spinning motor...");
      previousState = 1;
    }
    spinMotor(motorSpeed);
  } else {  //if the switch is off...
    if (previousState != 0) {
      Serial.println("Switch is off. Turning off motor");
      previousState = 0;
    }
